...
enum Color : uint8_t { White, Black, ColorCount = 2 };

constexpr Color operator~(Color c) {
    return Color(c ^ 1);
}

enum PieceType : uint8_t { Pawn, Knight, Bishop, Rook, Queen, King, PieceTypeCount = 6 };
...
constexpr int square_rank(Square s) {
    return s / 8;
}
constexpr int square_file(Square s) {
    return s % 8;
}
